...
function readWorkerConfig({ filePath = "agent-worker.yaml", serviceName, hostEnv = process.env } = {}) {
  const absolutePath = path.resolve(filePath);
  const filename = path.basename(absolutePath);
  if (!/^(?:\.env)(?:\.|$)/i.test(filename) && !/\.env$/i.test(filename)) {
    return readWorkerManifest({ filePath, serviceName, hostEnv });
  }
  if (serviceName !== undefined) throw new Error("--service is only supported with YAML manifests, not .env files.");
  let fileEnv;
  try {
    fileEnv = parseEnv(fs.readFileSync(absolutePath, "utf8"));
  } catch (error) {
    throw new Error(`Unable to read worker env file ${absolutePath}: ${error.message}`, { cause: error });
  }
  const env = { ...hostEnv, ...fileEnv };
  return {
    serviceName: env.WORKER_NAME || filename,
    directory: path.dirname(absolutePath),
    env,
  };
}

export { interpolate, readWorkerManifest, readWorkerConfig };
