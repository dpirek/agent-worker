...
function createOfficeConnection({
  ...
  if (!rejectUnauthorized && new URL(endpoint).protocol === "wss:") {
    throw new Error("The native WebSocket client requires certificate verification. Remove AI_HARNESS_OFFICE_TLS_REJECT_UNAUTHORIZED=false and use NODE_EXTRA_CA_CERTS to trust your Office certificate.");
  }
  ...
  function connect() {
    ...
    current.addEventListener("message", (event) => {
      try {
        ...
        if (payload.type === "task_cancel") {
          if (!activeConnectionId) throw new Error("The office cancelled a task before registration completed.");
          onTaskCancel(payload, { connectionId: activeConnectionId });
          return;
        }
