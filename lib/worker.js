...
const DEFAULT_TOOLS = ["list_files", "read_file", "write_file", "search_files", "curl", "run_command", "read_office_context"];
const AGENT_SKILLS = [{
  id: "coding-task",
  name: "Coding Task",
...
function workspaceArtifact(job) {
  if (!job.archiveUrl || !job.archive) return null;
  const name = `${job.taskId}.zip`;
  return {
    artifactId: `workspace-${job.taskId}`,
    name,
    parts: [{ kind: "file", file: { name, mimeType: "application/zip", uri: job.archiveUrl } }],
    metadata: { fileCount: job.archive.fileCount, size: job.archive.size },
  };
}
...
function createWorkerServer({
  ...
  function enqueue({ taskId = crypto.randomUUID(), messageId, text, internal = false, resourceBaseUrl, connectionId = null, sendUpdate = null }) {
    const job = {
      ...
      archiveUrl: `${String(resourceBaseUrl).replace(/\/$/, "")}/${taskId}.zip`,
  ...
  function completeControlTask({ taskId, messageId, text, connectionId, sendUpdate }) {
    const job = {
      ...
      executionControl: createTaskExecutionControl(), resourceBaseUrl: "", archiveUrl: "",
    };
  ...
  server.on("listening", () => {
    ...
    try {
      ...
      officeConnection = officeConnectionFactory({
        ...
        onInfo,
        onStatus(next) { orchestration = next; },
        ...
        onDirectMessage(payload, transport) {
          ...
          if (stopResult) {
            transport.send(directMessageResponse(messageId, stopResult));
            return;
          }
          directQueue.add({ messageId, text, sendResponse: transport.send });
        },
        onTask(payload, transport) {
          ...
          if (stopTaskCommand(text)) {
            completeControlTask({
              taskId, messageId, text, connectionId: transport.connectionId, sendUpdate: transport.send,
            });
            return;
          }
          enqueue({
