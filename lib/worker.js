...
async function createHarnessRunner({ env = process.env, onInfo = console.error } = {}) {
  ...
  return async function runTask(prompt, { resourceBaseUrl = "", workspace = workerRoot } = {}) {
...
function agentInfo(baseUrl, env = process.env) {
  const provider = providerConfiguration(env);
  const workerUrl = String(baseUrl).replace(/\/$/, "");
  return {
  ...
  const queue = createTaskQueue({
    ...
    async run(job) {
      ...
      try {
        ...
        persist(job);
        const text = await runner(job.text, { resourceBaseUrl: job.resourceBaseUrl, workspace: job.workspace });
        ...
        job.error = error.message;
        job.errorDetails = serializeError(error);
        if (job.workspace && !job.archive) {
          try {
            job.archive = await createWorkspaceZip(job.workspace, `${job.workspace}.zip`);
          } catch (archiveError) {
            job.errorDetails.archive = serializeError(archiveError);
          }
        }
  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && requestUrl.pathname === "/a2a") {
        ...
        const existing = jobs.get(messageId) || store.findByMessageId(messageId);
        if (existing) {
