import { instrumentModelClient, sanitizeActivity } from './activity.js';
import { testEnvironment, testRedactor } from './lab-config.js';
...
import { connectOfficeMcp, normalizeOfficeMcpServers, testOfficeMcpConnectivity, OFFICE_MCP_INSTRUCTIONS } from "./office-mcp.js";
...
function workspaceArtifact(job) {
  if (!job.upload?.uri || !job.archive) return null;
...
function resultMessage(job, state, text, error) {
  ...
  return {
    ...
    ...(state === 'completed' && job.upload?.artifactId ? { uploadedArtifactIds: [job.upload.artifactId] } : {}),
    ...(error ? { error } : {}),
  };
}
...
async function createHarnessRunner({ env = process.env, onInfo = console.error, modelClientFactory = createModelClient } = {}) {
  ...
  return async function runTask(prompt, { resourceBaseUrl = "", workspace = workerRoot, executionControl, officeMcpTools = [], officeHttpOrigin = "", projectId = "central-office", officeTaskId = null, onActivity = () => {} } = {}) {
    ...
    const localTools = createTools({ root, approve: approval, env: { ...env, AI_HARNESS_OFFICE_PROJECT_ID: projectId } }).filter((tool) => allowedNames.has(tool.name));
    ...
    const client = modelClientFactory({
      name: provider.name,
      url: provider.url,
      apiKey: provider.apiKey,
    });
    const report = entry => { try { onActivity(sanitizeActivity(entry)); } catch { /* Best-effort telemetry. */ } };
    instrumentModelClient(client, report, { provider: provider.name, agent: env.WORKER_NAME || 'Worker', projectId, taskId: officeTaskId });
    const systemPrompts = environmentSystemPrompts(env);
    if (officeMcpTools.length) systemPrompts.agent_instructions = `${systemPrompts.agent_instructions || DEFAULT_SYSTEM_PROMPTS.agent_instructions}\n\n${OFFICE_MCP_INSTRUCTIONS}\nOffice HTTP origin: ${officeHttpOrigin}`;
    const agent = new CodingAgent({
      client, tools: [...localTools, ...mcpTools, ...officeMcpTools], model: provider.model, root, approve: approval, onInfo,
      maxTurns: positiveInteger(env.WORKER_MAX_TURNS, 30),
      systemPrompts,
      resourceBaseUrl,
      onEvent(event) {
        if (['tool_start', 'tool_result', 'tool_blocked', 'mcp_call'].includes(event.type)) report({
          category: event.type === 'mcp_call' ? 'mcp' : 'tool', message: `${event.type.replaceAll('_', ' ')} · ${event.name || event.server || ''}`,
          tone: event.output?.ok === false || event.type === 'tool_blocked' ? 'error' : 'tool', metadata: event,
        });
...
function publicJob(job, includeResult = false) {
  return {
    ...
    mcp: job.mcp || null,
    ...(job.testRun ? { testRun: true, activity: job.activity || [] } : {}),
...
function createWorkerServer({
  env = process.env,
  runTask,
  testRunnerFactory = createHarnessRunner,
  ...
  let mcpProbeAbort;
  let mcpConnectivity = { status: 'waiting', toolCount: 0, error: null };

  function resetMcpConnectivity() {
    mcpProbeAbort?.abort();
    mcpProbeAbort = null;
    mcpConnectivity = { status: 'waiting', toolCount: 0, error: null };
  }
    ...
    async run(job) {
      ...
      if (!job.internal) {
        try {
          ...
          delete job.mcpServers;
          delete job.artifactUpload;
          persist(job);
          return;
        }
      }
      ...
      try {
        ...
        persist(job);
        const officeMcpServers = normalizeOfficeMcpServers(job.mcpServers, env.AI_HARNESS_OFFICE_URL, job.taskId);
        if (officeMcpServers.length) {
          officeMcp = await connectOfficeMcp({ servers: officeMcpServers, signal: job.executionControl.signal, fetchImpl,
            onStatus(next) { job.mcp = { ...job.mcp, ...next }; persist(job); } });
        }
        const taskRunner = job.testRun ? await testRunnerFactory({ env: job.testEnv, onInfo: message => {
          job.activity.push({ at: new Date().toISOString(), message: job.redact(String(message)).slice(0, 8000) });
          if (job.activity.length > 100) job.activity.shift();
        } }) : runner;
        job.executionControl.signal.throwIfAborted();
        const rawText = await taskRunner(job.text, {
          projectId: job.projectId, officeTaskId: job.officeTaskId,
          onActivity: event => {
            if (job.testRun) {
              job.activity.push({ at: new Date().toISOString(), category: event.category, message: job.redact(String(event.message || '')).slice(0, 8000) });
              if (job.activity.length > 100) job.activity.shift();
              event = { category: event.category, message: job.redact(String(event.message || '')) };
            }
            server.emit('activity', sanitizeActivity(event));
            if (!job.internal && !job.connectionClosed) job.sendUpdate?.({ type: 'activity', taskId: job.taskId, messageId: job.messageId, event });
          },
          resourceBaseUrl: job.resourceBaseUrl,
          workspace: job.workspace,
          executionControl: job.executionControl,
          signal: job.executionControl.signal,
          officeMcpTools: officeMcp?.tools || [],
          officeMcpServers,
          officeHttpOrigin: officeMcpServers.length ? new URL(officeMcpServers[0].url).origin : "",
        });
        const text = job.testRun ? job.redact(String(rawText)) : rawText;
        if (job.connectionClosed || job.stopped) return;
        ...
        if (!job.internal) {
          job.upload = await uploadWorkspace({
            ...
            artifactUpload: job.artifactUpload,
            messageId: job.messageId,
            env,
            fetchImpl,
            signal: job.executionControl.signal,
        ...
        if (job.connectionClosed || job.stopped || job.executionControl.signal.aborted) return;
        job.state = "completed";
        resultPayload = resultMessage(job, "completed", text);
      } catch (error) {
        if (job.connectionClosed || job.stopped) return;
        if (job.testRun) error = new Error(job.redact(error.message));
        job.state = "failed";
        job.error = error.message;
        job.errorDetails = serializeError(error);
        ...
        if (job.mcp?.status === "queued") job.mcp = { status: "error", toolCount: 0, error: error.message };
      } finally {
        await officeMcp?.close();
        delete job.mcpServers;
        delete job.artifactUpload;
        delete job.testEnv;
        if (["queued", "connecting", "connected"].includes(job.mcp?.status)) job.mcp = { ...job.mcp, status: "closed" };
        persist(job);
      }
  ...
  const directQueue = createTaskQueue({
    ...
    async run(message) {
      ...
      try {
        ...
        await fs.mkdir(workspace, { recursive: false });
        responseText = await runner(message.text, { resourceBaseUrl: "", workspace, projectId: message.projectId, officeTaskId: message.officeTaskId, onActivity: event => {
          server.emit('activity', sanitizeActivity(event));
          message.sendResponse({ type: 'activity', messageId: message.messageId, event });
        } });
  ...
  function enqueue({ taskId = crypto.randomUUID(), messageId, text, internal = false, resourceBaseUrl, connectionId = null, sendUpdate = null, mcpServers, artifactUpload, testEnv, projectId = "central-office", officeTaskId = null }) {
    ...
    const job = {
      ...
      mcpServers, artifactUpload, projectId, officeTaskId,
      ...(testEnv ? { testRun: true, testEnv, redact: testRedactor(testEnv), activity: [] } : {}),
      mcp: internal ? null : { status: mcpServers ? "queued" : "not_provided", toolCount: 0 },
    };
  ...
  function disconnectConnection(connectionId, reason) {
    if (connectionId === testedConnectionId) {
      resetMcpConnectivity();
  ...
  function status() {
    ...
    const recentTasks = store.listRecent(50);
    const activeMcp = recentTasks.filter(task => task.source === "office" && ["submitted", "working"].includes(task.state)).map(task => ({ taskId: task.taskId, ...task.mcp }));
    const connectedMcp = activeMcp.filter(task => task.status === "connected");
    const activeMcpError = activeMcp.find(task => task.status === "error");
    const lastMcp = recentTasks.find(task => task.mcp && task.mcp.status !== "not_provided")?.mcp;
    const mcpError = activeMcpError || (!activeMcp.length && lastMcp?.status === "error" ? lastMcp : null);
    const mcpStatus = orchestration.status !== "connected" ? "disconnected"
      : activeMcpError ? "error" : connectedMcp.length ? "connected"
        : activeMcp.some(task => ["queued", "connecting"].includes(task.status)) ? "connecting"
          : activeMcp.length ? "not_provided" : mcpError ? "error" : mcpConnectivity.status;
    return {
      ok: true,
      agent: {
      ...
      execution: {
        ...
        officeMcp: { status: mcpStatus, automatic: true, activeTasks: activeMcp,
          connectedTasks: connectedMcp.length, toolCount: connectedMcp.reduce((sum, task) => sum + (task.toolCount || 0), 0),
          connectivity: { ...mcpConnectivity },
          error: mcpError?.error || mcpConnectivity.error || null },
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (['GET', 'HEAD'].includes(req.method) && ['/test', '/test/'].includes(requestUrl.pathname)) {
        await serveStatic(req, res, publicDir, '/test.html');
        return;
      }
      if (req.method === 'GET' && requestUrl.pathname === '/api/test/config') {
        const provider = providerConfiguration(env);
        respondJson(res, { envConfig: [
          `PROVIDER_NAME=${JSON.stringify(provider.name)}`, `PROVIDER_URL=${JSON.stringify(provider.url)}`,
          `PROVIDER_MODEL=${JSON.stringify(provider.model)}`, '# PROVIDER_API_KEY= (omit to use the running worker key)',
          `WORKER_MAX_TURNS=${positiveInteger(env.WORKER_MAX_TURNS, 30)}`, `WORKER_TOOLS=${configuredTools(env).join(',')}`,
        ].join('\n'), apiKeyConfigured: Boolean(provider.apiKey) });
        return;
      }
      if (req.method === 'POST' && requestUrl.pathname === '/api/test/cancel') {
        if (!allowedControlOrigin(req.headers.origin, requestUrl, env.WORKER_PUBLIC_URL)) throw Object.assign(new Error('Cross-origin test requests are not allowed.'), {statusCode:403});
        const {taskId} = await readJson(req, 1024);
        const job = tasks.get(taskId);
        if (!job?.testRun) throw new Error('Test run not found.');
        stopJob(job, 'Test stopped by user.');
        delete job.testEnv;
      ...
      if (req.method === "POST" && requestUrl.pathname === "/api/test") {
        ...
        const job = enqueue({
          ...
          text,
          internal: true,
          ...(payload.envConfig !== undefined ? { testEnv: testEnvironment(payload.envConfig, env) } : {}),
    ...
    try {
      ...
      officeConnection = officeConnectionFactory({
        ...
        onStatus(next) {
          ...
          orchestration = next;
          if (next.status !== 'connected') resetMcpConnectivity();
          ...
          mcpProbeAbort = probe;
          mcpConnectivity = { status: 'connecting', toolCount: 0, error: null };
          void testOfficeMcpConnectivity({ env, fetchImpl,
            signal: AbortSignal.any([probe.signal, connectivityAbort.signal]),
          }).then(result => {
            if (probe.signal.aborted || generation !== officeGeneration) return;
            mcpConnectivity = result;
            onInfo(`Office MCP connectivity test succeeded: project context accessible, ${result.toolCount} tools discovered.`);
          }, error => {
            if (probe.signal.aborted || generation !== officeGeneration) return;
            mcpConnectivity = { status: 'error', toolCount: 0, error: error.message };
            onInfo(`Office MCP connectivity test failed: ${error.message}`);
          });
        ...
        onTaskCancel(payload, transport) {
          const job = tasks.get(payload.taskId);
          if (!job || job.connectionId !== transport.connectionId || !["submitted", "working"].includes(job.state)
            || (payload.inReplyTo && payload.inReplyTo !== job.messageId)) return;
          const reason = String(payload.reason || "Task cancelled by Office.");
          job.stopped = true;
          delete job.mcpServers;
          delete job.artifactUpload;
          job.executionControl.stop(reason);
        ...
        onDirectMessage(payload, transport) {
          ...
          directQueue.add({ messageId, text, sendResponse: send, projectId: payload.projectId || payload.context?.project?.id || "central-office", officeTaskId: payload.officeTaskId || payload.context?.task?.id || null });
        },
        onTask(payload, transport) {
          ...
          enqueue({
            ...
            artifactUpload: payload.artifactUpload,
            projectId: payload.projectId || payload.context?.project?.id || "central-office",
            officeTaskId: payload.officeTaskId || payload.context?.task?.id || null,
          });
        },
      });
