...
async function createHarnessRunner({ env = process.env, onInfo = console.error, modelClientFactory = createModelClient } = {}) {
  ...
  return async function runTask(prompt, { resourceBaseUrl = "", workspace = workerRoot, executionControl, officeMcpTools = [], officeHttpOrigin = "", projectId = "central-office", officeTaskId = null } = {}) {
  ...
  const directQueue = createTaskQueue({
    ...
    async run(message) {
      ...
      try {
        ...
        await fs.mkdir(workspace, { recursive: false });
        responseText = await runner(message.text, { resourceBaseUrl: "", workspace, projectId: message.projectId, officeTaskId: message.officeTaskId });
