    ...
    async run(job) {
      ...
      try {
        ...
        const text = await runner(job.text, {
          ...
          onActivity: event => { if (!job.internal && !job.connectionClosed) job.sendUpdate?.({ type: 'activity', taskId: job.taskId, messageId: job.messageId, event }); },
  ...
  const directQueue = createTaskQueue({
    ...
    async run(message) {
      ...
      try {
        ...
        await fs.mkdir(workspace, { recursive: false });
        responseText = await runner(message.text, { resourceBaseUrl: "", workspace, projectId: message.projectId, officeTaskId: message.officeTaskId, onActivity: event => message.sendResponse({ type: 'activity', messageId: message.messageId, event }) });
