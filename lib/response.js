...
export async function serveStatic(req, res, publicDir, requestedPath) {
  let requestUrl;
  try {
  ...
  const urlPath = requestedPath ?? requestUrl.pathname;
  if (requestedPath === undefined && urlPath === "/favicon.ico") {
    redirect(res, "/logo.svg");
    return;
  }

  const filePath = resolvePublicPath(publicDir, urlPath);
  if (!filePath) {
  ...
  try {
    ...
    if (realRelative.startsWith("..") || path.isAbsolute(realRelative)) {
      respondJson(res, { ok: false, error: "Path is outside served directory." }, 403);
      return;
    }
    ...
    respond(res, 200, await fs.readFile(realFilePath), {
      type: contentType(realFilePath),
