...
export function createWebSocketHandler({
  ...
  return function handleWebSocket(socket, req) {
      ...
      try {
        ...
        for (const message of decoded.messages) {
          ...
          if (payload.type === "provider_settings") {
            const name = normalizeProvider(payload.name);
            ...
            agentSessions.clear();
            streamStates.clear();
            const { apiKey: _secret, ...visibleProviderSettings } = providerSettings;
            emit({
    ...
    const configuredProvider = providerConfiguration();
    const { apiKey: _secret, ...visibleProvider } = configuredProvider;
    emit({
      type: "ready",
