...
class ChatCompletionsClient {
  ...
  async createResponse(body, { onTextDelta, signal } = {}) {
    const messages = this.requestMessages(body);
    if (onTextDelta) return this.createStreamingResponse(body, messages, onTextDelta, signal);
  ...
  async createStreamingResponse(body, messages, onTextDelta, signal) {
    const url = `${this.baseUrl}/chat/completions`;
    const options = {
      method: "POST",
      headers: this.headers(),
