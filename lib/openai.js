...
class ChatCompletionsClient {
  ...
  async createResponse(body, { onTextDelta } = {}) {
    ...
    try {
      ...
      throw new Error(`${this.providerName} returned invalid JSON (HTTP ${response.status}): ${raw}`);
    }
    if (!response.ok) {
      throw new Error(`${this.providerName} API error (HTTP ${response.status}): ${data.error?.message || raw}`);
    }
