...
function createReadOfficeContextTool({ env = process.env, fetchImpl = fetch } = {}) {
  ...
  return {
    ...
    parameters: objectSchema({
      projectId: { type: ["string", "null"], description: "Project this request relates to; null uses the assigned project." },
      view: {
        type: "string",
    ...
    async execute({ view, query, full_details: fullDetails, limit, projectId }, { signal } = {}) {
      const assignedProject = env.AI_HARNESS_OFFICE_PROJECT_ID;
      if (assignedProject && projectId && projectId !== assignedProject) throw new Error("Requested project does not match this message's project.");
      const scopedProject = projectId || assignedProject || "central-office";
      const boundedLimit = Math.max(1, Math.min(100, Number(limit) || 20));
      if (view === "completed_tasks") {
        const payload = await request("/api/memory", {
          projectId: scopedProject, kind: "task", status: "completed", query: query?.trim(), limit: boundedLimit,
        }, signal);
      ...
      if (view === "task_details") {
        ...
        const payload = await request("/api/tasks", { projectId: scopedProject }, signal);
      ...
      if (view === "messages") {
        const payload = await request("/api/chat", { projectId: scopedProject, limit: boundedLimit }, signal);
