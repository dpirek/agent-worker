...
const PROMPT_ENV_KEYS = {
  ...
  response_format: "AI_HARNESS_RESPONSE_FORMAT",
...
export function environmentSystemPrompts(env = process.env) {
  return Object.fromEntries(
    Object.entries(PROMPT_ENV_KEYS)
      .filter(([, key]) => Object.hasOwn(env, key))
      .map(([name, key]) => [name, String(env[key])]),
  );
}
...
export function applyEnvironmentSettings(uiStateStore, env = process.env, root = process.cwd()) {
  ...
  if (env.PROVIDER_NAME?.trim()) next.providerSettings.provider = env.PROVIDER_NAME.trim();
  if (env.PROVIDER_MODEL?.trim()) next.providerSettings.model = env.PROVIDER_MODEL.trim();
  if (Object.hasOwn(env, "PROVIDER_URL")) next.providerSettings.baseUrl = String(env.PROVIDER_URL).trim();
  if (Object.hasOwn(env, "PROVIDER_API_KEY")) next.providerSettings.apiKey = String(env.PROVIDER_API_KEY).trim();
  ...
  Object.assign(next.systemPrompts, environmentSystemPrompts(env));
