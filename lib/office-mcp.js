...
export async function testOfficeMcpConnectivity({ env, signal, fetchImpl = fetch }) {
  const token = String(env.AI_HARNESS_WORKER_TOKEN || '').trim();
  if (!token) throw new Error('Office MCP connectivity test requires the worker token.');
  // Office accepts worker authentication on this read-only endpoint even before
  // an assignment exists. Never attach this broader credential to task tools.
  const taskId = 'worker-connectivity';
  const servers = normalizeOfficeMcpServers({ office_project: {
    type: 'http', url: `/mcp/projects/central-office/tasks/${taskId}`,
    headers: { Authorization: `Bearer ${token}` },
  } }, env.AI_HARNESS_OFFICE_URL, taskId);
  const timeout = AbortSignal.timeout(30_000);
  const session = await connectOfficeMcp({ servers, fetchImpl,
    signal: signal ? AbortSignal.any([signal, timeout]) : timeout });
  try {
    const tool = session.tools.find(tool => tool.name === 'office_project__project_get_context');
    if (!tool) throw new Error('Office MCP connectivity test requires project_get_context.');
    const result = await tool.execute({});
    if (!result || result.isError || !Array.isArray(result.content)) {
      throw new Error('Office MCP project-context connectivity test failed.');
    }
    return { status: 'verified', toolCount: session.tools.length, verifiedAt: new Date().toISOString(), error: null };
