...
function environmentInstructions(
  ...
  const promptSections = [
    ...
    disabledSteps.has("response") ? "" : prompts.response_format,
  ];
  const toolNames = new Set(tools.map((tool) => tool.name));
  if (toolNames.has("list_teammates") && toolNames.has("ask_teammate")) {
    promptSections.splice(3, 0, `Office collaboration:
When a task would materially benefit from another teammate's advertised expertise, call
list_teammates to compare capabilities, then ask_teammate with a bounded, self-contained request.
Choose teammates by relevant skills and tools, do not delegate to yourself, avoid duplicate or
circular requests, and remain responsible for checking and integrating their advisory response.`);
  }
