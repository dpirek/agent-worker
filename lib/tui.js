...
export function renderDashboard(status, { columns = 80, rows = 24, logs = [], offset = 0,
  paused = false, now = Date.now(), startedAt = now, address = '' } = {}) {
  ...
  return body.map(line => clip(line, width)).join('\r\n');
}
  ...
  const draw = () => {
    ...
    try {
      ...
      output.write('\x1b[H' + renderDashboard(snapshot, {
        columns: output.columns, rows: output.rows, logs, offset, paused, startedAt, address,
